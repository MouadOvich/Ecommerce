...
class ProduitRepo{
    ...
    public function getAll($id=null){
        ...
        if ($id==null) {
            $sql = "SELECT * FROM produit p, categorie c WHERE p.id_cat=c.id_cat";
        } else {
            $sql = "SELECT * FROM produit p, categorie c WHERE p.id_cat=c.id_cat AND (p.id_cat=$id OR c.sous_categorie=$id)";
        }
    ...
    public function deleteProd($id){
        ...
        $stm = $connexion->prepare($sql);
        $lines=$stm->execute([$id]);
        return ($lines>0);
    }

    public function getImgName($id){
        $connexion=$this->connexion;
        $sql = "SELECT image1 FROM produit WHERE visibilite=1 AND id_prod=$id ";
        $stm = $connexion->prepare($sql);
        $stm->execute();
        $list_products = $stm->fetch();
        return $list_products;
